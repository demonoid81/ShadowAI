...
func (r *PGRepository) GetActive(ctx context.Context) (*Policy, error) {
	...
	const q = `SELECT id, name, mode, rules_json, updated_at, updated_by, is_active
	...
	err := r.db.QueryRowContext(ctx, q).Scan(
		&p.ID, &p.Name, &p.Mode, &rulesJSON, &p.UpdatedAt, &updatedBy, &p.IsActive,
	)
	if err != nil {
...
func (r *PGRepository) Upsert(ctx context.Context, p *Policy, actor string) (*Policy, error) {
	...
	case errors.Is(err, sql.ErrNoRows):
		const ins = `INSERT INTO provider_governance_policies
		    (name, mode, rules_json, updated_at, updated_by, is_active)
		    VALUES ($1, $2, $3, now(), $4, true)
		    RETURNING id, updated_at`
		var newID string
		var updAt time.Time
		if err := r.db.QueryRowContext(ctx, ins,
			name, string(p.Mode), rulesJSON, actorArg,
	...
	case err == nil:
		const upd = `UPDATE provider_governance_policies
		    SET name = $1, mode = $2, rules_json = $3,
		        updated_at = now(), updated_by = $4
		    WHERE id = $5
		    RETURNING updated_at`
		var updAt time.Time
		if err := r.db.QueryRowContext(ctx, upd,
			name, string(p.Mode), rulesJSON, actorArg, existingID,
		).Scan(&updAt); err != nil {
