...
| Legal hold query-scope not implemented | `whole_user` and `date_range` holds are enforced; arbitrary query-scope selectors are not implemented | Use `whole_user` or `date_range`; `query_scope` returns a validation error. Design is captured in PR-L8 RFC; implementation remains future work |
